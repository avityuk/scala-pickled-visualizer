...
h2. Running application

Diagram rendering is done with ["Graphviz tool":http://www.graphviz.org]. It should be installed and available in command line.

h3. Demo

There are two demo classes (__TestClass1__, __TestClass2__) provided. In order to generate diagrams for them you can use **run-demo.sh** script.

h3. Custom class file

**run.sh classpath class_name**

h2. Pickle format

Information taken from __scala.reflect.PickleFormat__.
...
| AnnotArg | <notextile>Tree OR Constant |
| ConstAnnotArg | Constant OR AnnotInfo OR AnnotArgArray |
